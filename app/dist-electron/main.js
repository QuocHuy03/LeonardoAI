...
async function getDb() {
  if (db) return db;
  const wasmPath = path.join(path.dirname(require2$1.resolve("sql.js")), "sql-wasm.wasm");
...
async function fetchJson(url, method, headers2, body) {
  const resp = await fetch(url, { method, headers: { ...BASE_HEADERS, ...headers2 }, body });
...
function registerIpc() {
  ...
  ipcMain.handle("generate:run", async (event, jobs) => {
    const send = (rowIdx, status, url = "") => event.sender.send("generate:progress", { rowIdx, status, url });
  ...
  ipcMain.handle("file:saveImage", async (_, url, suggestedName) => {
...
function createWindow() {
  const iconPath = path.join(__dirname$1, "../logo.png");
