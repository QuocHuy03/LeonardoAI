...
util$3.exports.Constants = constants;
...
const httpAdapter = isHttpAdapterSupported && function httpAdapter2(config2) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    if (supportedProtocols.indexOf(protocol2) === -1) {
      return reject(
        new AxiosError$1("Unsupported protocol " + protocol2, AxiosError$1.ERR_BAD_REQUEST, config2)
      );
    }
...
let Axios$1 = class Axios {
  ...
  _request(configOrUrl, config2) {
    ...
    headers2 && utils$1.forEach(["delete", "get", "head", "post", "put", "patch", "common"], (method) => {
...
(function(module, exports$1) {
  !function(t, n) {
    module.exports = n(require$$0$3, require$$1);
...
const initSqlJs = require2$1("sql.js");
let db = null;
function getDbPath() {
  return path$1.join(app.getPath("userData"), "leonardo.db");
}
async function getDb() {
  if (db) return db;
  const wasmPath = path$1.join(path$1.dirname(require2$1.resolve("sql.js")), "sql-wasm.wasm");
  const SQL = await initSqlJs({ wasmBinary: fs$1.readFileSync(wasmPath) });
  const dbPath = getDbPath();
  if (fs$1.existsSync(dbPath)) {
    db = new SQL.Database(fs$1.readFileSync(dbPath));
  } else {
    db = new SQL.Database();
  }
  initSchema();
...
function saveDb() {
  ...
  fs$1.writeFileSync(getDbPath(), Buffer.from(data));
}
function initSchema() {
  db.run(`
  ...
  `);
  try {
    db.run(`ALTER TABLE generations ADD COLUMN project_id INTEGER DEFAULT NULL`);
  } catch {
  }
  saveDb();
}
async function addCookie(cookieStr) {
  var _a;
  const d = await getDb();
  d.run(
  ...
  return (_a = result[0]) == null ? void 0 : _a.values[0][0];
}
...
async function updateCookieTokens(id, tokens, email, userId, status) {
  const d = await getDb();
...
async function getCookieById(id) {
  const d = await getDb();
...
async function getAllCookieStrings() {
  const d = await getDb();
  const res = d.exec(`SELECT id, cookie_str FROM cookies WHERE status != 'EMPTY'`);
  if (!res.length) return [];
  return res[0].values.map((r) => ({ id: r[0], cookie_str: r[1] }));
}
...
async function listProjects() {
  ...
  return res[0].values.map(
    (r) => Object.fromEntries(cols.map((c, i) => [c, r[i]]))
  );
}
...
async function logGeneration(cookieId, prompt, modelId, genId, projectId) {
  try {
    const d = await getDb();
    d.run(
...
async function updateGenStatus(genId, status, imageUrl) {
  const d = await getDb();
...
async function listGenerations(projectId) {
  try {
    const d = await getDb();
    let stmt;
    let rows;
...
async function createCharacter(name, description, imagePath) {
  var _a;
  const d = await getDb();
...
async function fetchJson(url2, method, headers2, body) {
  const resp = await fetch(url2, { method, headers: { ...BASE_HEADERS, ...headers2 }, body });
...
async function getTokenFromCookie(cookieStr) {
  let csrf;
  ...
  const cookieHeader = cookieStr.split(";").map((s) => s.trim()).filter((s) => s.includes("=")).join("; ");
  const headers2 = {
    ...BASE_HEADERS,
    cookie: cookieHeader
  };
  if (csrf) {
    try {
      const r = await fetchJson(`${AUTH_URL}/session`, "POST", headers2, JSON.stringify({ csrfToken: csrf }));
      const token = (r == null ? void 0 : r.idToken) || (r == null ? void 0 : r.accessToken);
      if (token) return token;
    } catch {
    }
  }
  try {
    const r = await fetchJson(`${AUTH_URL}/session`, "GET", headers2);
    return (r == null ? void 0 : r.idToken) || (r == null ? void 0 : r.accessToken) || null;
  } catch {
  }
  return null;
}
async function getUserInfo(token) {
  ...
  try {
    ...
    const users = ((_a = rj == null ? void 0 : rj.data) == null ? void 0 : _a.users) || [];
    if (users.length) {
      const u = users[0];
      const d = ((_b = u.user_details) == null ? void 0 : _b[0]) || {};
      return {
        userId: u.id,
        email: d.auth0Email || "",
  ...
  try {
    ...
    const d = ((_d = (_c = rj == null ? void 0 : rj.data) == null ? void 0 : _c.user_details) == null ? void 0 : _d[0]) || {};
    return { userId: "", email: "", tokens: (d.subscriptionTokens || 0) + (d.paidTokens || 0) + (d.rolloverTokens || 0) };
  } catch {
  }
  return { userId: "", email: "", tokens: 0 };
}
async function uploadImagePath(token, filePath) {
  var _a, _b;
  const fs2 = await import("fs");
  const path2 = await import("path");
  const ext = path2.extname(filePath).slice(1).toLowerCase() || "jpg";
  ...
  const rj = await gql(token, q);
  const ud = (_a = rj == null ? void 0 : rj.data) == null ? void 0 : _a.uploadImage;
  if (!ud) throw new Error("UploadImage mutation failed");
  const uploadId = ud.uploadId;
  const s3Url = ud.url;
...
`
  ...
  parts.push(Buffer.from(
    `--${boundary}\r
Content-Disposition: form-data; name="file"; filename="${path2.basename(filePath)}"\r
Content-Type: ${ct}\r
\r
`
  ));
...
`));
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const records = ((_b = r == null ? void 0 : r.data) == null ? void 0 : _b.init_image_moderation) || [];
    if (records.length) {
      const { checkStatus, initImageId } = records[0];
      if (checkStatus === "Accepted" && initImageId) return initImageId;
      if (checkStatus === "Rejected") throw new Error("Image rejected by moderation");
    }
  }
  throw new Error("Moderation timeout");
}
async function createGeneration(token, prompt, modelId, apiType, width, height, quantity, initImageIds) {
  ...
  if (apiType === "NEW") {
    ...
    if (initImageIds == null ? void 0 : initImageIds.length) {
    ...
    const rj = await gql(token, q);
    const genId = (_b = (_a = rj == null ? void 0 : rj.data) == null ? void 0 : _a.generate) == null ? void 0 : _b.generationId;
    if (genId) return genId;
    const errs = (_c = rj == null ? void 0 : rj.errors) == null ? void 0 : _c.map((e) => e.message).join(", ");
    ...
    const rj = await gql(token, q);
    const genId = (_e = (_d = rj == null ? void 0 : rj.data) == null ? void 0 : _d.sdGenerationJob) == null ? void 0 : _e.generationId;
    if (genId) return genId;
    const errs = (_f = rj == null ? void 0 : rj.errors) == null ? void 0 : _f.map((e) => e.message).join(", ");
...
async function pollStatus(token, genId) {
  ...
  const rj = await gql(token, q);
  return ((_c = (_b = (_a = rj == null ? void 0 : rj.data) == null ? void 0 : _a.generations) == null ? void 0 : _b[0]) == null ? void 0 : _c.status) || "PENDING";
}
async function getImageUrls(token, genId) {
  var _a, _b, _c;
  const q = {
  ...
  const rj = await gql(token, q);
  const imgs = ((_c = (_b = (_a = rj == null ? void 0 : rj.data) == null ? void 0 : _a.generations) == null ? void 0 : _b[0]) == null ? void 0 : _c.generated_images) || [];
...
function registerIpc() {
  ipcMain.handle("cookies:add", async (_, cookieLines) => {
    const lines = cookieLines.split("\n").map((s) => s.trim()).filter(Boolean);
    const ids = [];
    for (const line of lines) {
      const id = await addCookie(line);
      ids.push(id);
    }
  ...
  ipcMain.handle("cookies:addFile", async () => {
    ...
    const content = fs$1.readFileSync(filePaths[0], "utf-8");
  ...
  ipcMain.handle("cookies:delete", async (_, id) => {
  ...
  ipcMain.handle("prompts:loadFile", async () => {
    ...
    return fs$1.readFileSync(filePaths[0], "utf-8");
  });
  ...
  ipcMain.handle("image:upload", async (event, cookieId, filePath) => {
  ...
  ipcMain.handle("generate:create", async (_, cookieId, prompt, modelId, apiType, width, height, quantity, initImageIds) => {
    const row = await getCookieById(cookieId);
    if (!row) throw new Error("Cookie not found");
    const token = await getTokenFromCookie(row.cookie_str);
  ...
  ipcMain.handle("generate:poll", async (_, cookieId, genId) => {
    const row = await getCookieById(cookieId);
    if (!row) throw new Error("Cookie not found");
    const token = await getTokenFromCookie(row.cookie_str);
    if (!token) throw new Error("Cannot get session token");
  ...
  ipcMain.handle("generate:getUrl", async (_, cookieId, genId) => {
    const row = await getCookieById(cookieId);
    if (!row) throw new Error("Cookie not found");
    const token = await getTokenFromCookie(row.cookie_str);
    if (!token) throw new Error("Cannot get session token");
  ...
  ipcMain.handle("generate:run", async (event, jobs) => {
    const send = (rowIdx, status, url2 = "") => event.sender.send("generate:progress", { rowIdx, status, url: url2 });
    await Promise.all(jobs.map(async (job) => {
      send(job.rowIdx, "Generating…");
      try {
        const row = await getCookieById(job.cookieId);
        if (!row) {
          send(job.rowIdx, "❌ Cookie not found");
          return;
        }
        const token = await getTokenFromCookie(row.cookie_str);
        if (!token) {
          send(job.rowIdx, "❌ Auth failed");
          return;
        }
        ...
        if (status === "COMPLETE") {
          ...
          urls.forEach((url2, i) => send(job.rowIdx + i, "✅ Done", url2));
        } else {
  ...
  ipcMain.handle("generations:list", async (_, projectId) => {
    return listGenerations(projectId);
  });
  ipcMain.handle("generations:delete", async (_, id) => {
  ...
  ipcMain.handle("characters:create", async (_, name, description, imagePath) => {
    const { app: app2 } = await import("electron");
    const destDir = path$1.join(app2.getPath("userData"), "characters");
    if (!fs$1.existsSync(destDir)) fs$1.mkdirSync(destDir, { recursive: true });
    const ext = path$1.extname(imagePath);
    const dest = path$1.join(destDir, `${Date.now()}${ext}`);
    fs$1.copyFileSync(imagePath, dest);
  ...
  ipcMain.handle("file:readImage", async (_, filePath) => {
    try {
      ...
      return `data:${mime2};base64,${buf.toString("base64")}`;
    } catch {
      return null;
    }
  });
  ...
  ipcMain.handle("folder:scan", async (_, folderPath) => {
    const IMG_EXT = /* @__PURE__ */ new Set([".jpg", ".jpeg", ".png", ".webp"]);
    const entries = fs$1.readdirSync(folderPath).sort();
    const images = entries.filter((f) => IMG_EXT.has(path$1.extname(f).toLowerCase())).map((f) => path$1.join(folderPath, f));
    let prompts = [];
    const promptFile = path$1.join(folderPath, "prompts.txt");
    if (fs$1.existsSync(promptFile)) {
      prompts = fs$1.readFileSync(promptFile, "utf8").split("\n").map((l) => l.trim()).filter((l) => l.length > 0);
    }
    return { images, prompts };
  });
  ipcMain.handle("file:saveImage", async (_, url2, suggestedName) => {
    ...
    fs$1.writeFileSync(filePath, buf);
    return { ok: true, path: filePath };
  });
  ipcMain.handle("file:saveZip", async (_, entries, zipName) => {
    const { filePath } = await dialog.showSaveDialog(getWin(), {
  ...
  const AUTH_URL2 = "https://serverkey.taothaoai.com/api/leonardo_ai/auth";
  ipcMain.handle("auth:verify", async () => {
    var _a, _b;
    try {
      const key = await getSetting("auth_key");
      if (!key) return { success: false };
      const device_id = distExports.machineIdSync();
      const response = await axios.post(AUTH_URL2, { key, device_id });
      ...
      return { success: false, error: ((_b = (_a = error.response) == null ? void 0 : _a.data) == null ? void 0 : _b.message) || error.message };
    }
  });
  ...
  ipcMain.handle("auth:get-device-id", () => distExports.machineIdSync());
...
function createWindow() {
  const iconPath = path$1.join(__dirname$1, "../logo.png");
